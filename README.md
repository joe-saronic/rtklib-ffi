...
### Raw Receiver Decoding - `raw` feature

**`rcvraw.c`**

Generic frame decoders used by all receiver-specific decoders.

- [ ] `init_raw` - initialize raw receiver control struct
- [ ] `free_raw` - free raw receiver control struct
...
- [ ] `init_sbf` / `free_sbf` - initialize/free Septentrio SBF struct
- [ ] `input_sbf` / `input_sbff` - Septentrio SBF decoder
